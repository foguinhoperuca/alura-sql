...
$EmailSubject = "Alerta: Consultas Longas no PostgreSQL"
$EmailBody = "ALERTA: Consultas ultrapassaram o limite de $Limite segundos."
$SMTPServer = "smtp.gmail.com"
$SMTPPort = 587
$SMTPUser = "seuemail@gmail.com"
$SMTPPassword = "suasenhaouappsenha"
...
if ($search_result) {
    ...
    # Atualizar o corpo do e-mail com o número de consultas que ultrapassaram o limite
    $EmailBody = "ALERTA: $count_result consultas ultrapassaram o limite de $Limite segundos."
    Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $EmailSubject -Body $EmailBody -SmtpServer $SMTPServer -Port $SMTPPort -Credential (New-Object PSCredential($SMTPUser, (ConvertTo-SecureString $SMTPPassword -AsPlainText -Force))) -UseSsl

    # Atualizar campo checkquery para "CHECK"
    $update_sql = "UPDATE query_logs SET checkquery = 'CHECK' WHERE duration_s > ($Limite) AND checkquery = 'NOTCHECK';"
