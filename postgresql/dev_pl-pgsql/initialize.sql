...
CREATE OR REPLACE TRIGGER tg_create_instructor_logs
  ...
  FOR EACH ROW EXECUTE FUNCTION dev_plpgsql.fn_create_instructor_trigger()
;

CREATE OR REPLACE FUNCTION dev_plpgsql.internal_instructors(IN instructor_id INTEGER) RETURNS refcursor LANGUAGE plpgsql AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION dev_plpgsql.fn_create_course_cursor() RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  v_salary_avarage DECIMAL;
  v_poorest_instructors INTEGER DEFAULT 0;
  v_total_instructors INTEGER DEFAULT 0;
...
BEGIN
  ...
  IF NEW.salary > v_salary_avarage THEN
    INSERT INTO dev_plpgsql.instructor_logs (information) VALUES (NEW.instructor_name || ' has better salary than avarage.');
  END IF;

  SELECT dev_plpgsql.internal_instructors(NEW.id) INTO v_cursor_salaries;
  LOOP
    FETCH v_cursor_salaries INTO v_salary;
    EXIT WHEN NOT FOUND;
    v_total_instructors := v_total_instructors + 1;
  ...
  INSERT INTO dev_plpgsql.instructor_logs (information) VALUES (NEW.instructor_name || ' has better salary than ' || v_percentage || '% of all instructors.');

  RETURN NEW;  
END;
$$;

CREATE OR REPLACE TRIGGER tg_create_instructor_logs_cursor
  AFTER INSERT ON dev_plpgsql.instructors
  FOR EACH ROW EXECUTE FUNCTION dev_plpgsql.fn_create_course_cursor()
;
